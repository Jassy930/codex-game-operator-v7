...
v0.7 `归航航线` 继续保持纯派生读回。本轮只新增阶段目标 `stageGoal`：未贯通时显示当前段、完成段数和进入下一段的缺口；三段贯通后显示储备数量。完整行动、收益、节奏、航线图和预览仍留在现有 `共鸣矩阵` 详情中。

约束：不修改归航奖励、余辉公式或 50 星尘上限；不新增资源、节点、等级树、任务系统、面板、存档字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-route-completion-reserve`

v0.7 `归航航线` 继续保持纯派生读回。本轮只调整三段贯通后的完成态：`routeProgress`、`routeSummary` 和 `currentPayoff` 会显示当前累计归航次数和额外共鸣储备，例如 `长期储备：6 次归航 / 5 点额外共鸣`。阶段目标继续保持短提示，避免主目标行膨胀。后续已由 `DECISION:2026-05-09-v07-completed-route-beacon` 把完成态命名收敛为 `深空信标储备`。
