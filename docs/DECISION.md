...
v0.5 `星尘归航` 已能形成可重复长线循环，但归航后的第 2 轮开局必须立即读回“这次归航带来了可花费的共鸣”，否则玩家会把归航误解成单纯重置。
...
- `CONTENT_REVIEW`: 复核重复归航节奏和满节点后的长期目标是否清楚。
- `PLAYABLE_CONTENT`: 如果 review 发现明确 gap，只调整既有读回或既有节点效果。
