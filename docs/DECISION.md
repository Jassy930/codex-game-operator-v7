...
`归航航线` 的信息已经足够完整，当前优先级是让重复归航的操作位置也能读懂目标：玩家在 `归航台` 准备点击时，应看到当前航线段、本段进度和下一步行动，而不必回扫 `共鸣矩阵`。这比继续追加新字段更符合 North Star 的“我知道下一步要做什么”。

## Active Decisions

### `DECISION:2026-05-09-v07-return-dock-route-readback`

v0.7 `归航航线` 在 `归航台` 增加一行操作位置短读：显示当前航线段、`completed/3`、本段进度和下一步行动。完整差距、节奏、航线图和下一段预览仍留在 `共鸣矩阵` 的 `航线细节`，归航台只负责让玩家点击归航前确认这次操作正在推进什么。

约束：不修改 `ReturnRouteReadback` 数值语义、归航奖励、存档、余辉公式、50 星尘上限或航线门槛；不新增资源、节点、等级树、任务系统、面板、存档字段、指标字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-route-detail-disclosure`

v0.7 `归航航线` 不再把十行正文全部铺在 `共鸣矩阵` 首层。首层只显示航线摘要、当前收益和下一步行动；本段进度、下一段门槛、差距、节奏预判、航线图和下一段预览仍保留在同一个航线块的 `航线细节` 中。

约束：不修改 `ReturnRouteReadback` 的数值语义、归航奖励、存档、余辉公式、50 星尘上限或航线门槛；不新增资源、节点、等级树、任务系统、面板、存档字段、指标字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-home-goal-focus`

首页只做展示层减法：右侧 `下一项工程` 改为 `当前目标`，主文案直接显示玩家下一步动作和差额；升级卡移除重复解释段，只保留名称、数值、按钮和差额；首页移除 `未来系统` 说明卡，研究所 / 日志只保留为锁定导航入口。

约束：不修改 `GameState`、存档、采集、升级、共鸣、归航、余辉、航线门槛、导入 / 重置逻辑或 telemetry；不新增面板、资源、节点、任务系统或视觉资产。

### `DECISION:2026-05-09-v07-route-transition-event`

`星尘归航` 完成事件现在接收归航前后状态：如果本次归航刚好把 `归航航线` 推入下一段，事件记录改为 `归航航线推进：进入...`，并读回当前段收益；如果没有跨段，继续显示当前段和本段进度。这样事件流负责“本次归航发生了什么”，`共鸣矩阵` 继续负责完整行动、差距、预览和航线图。

约束：不修改归航奖励、存档、余辉公式、50 星尘上限或航线门槛；不新增资源、节点、等级树、任务系统、面板、存档字段、指标字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-mobile-bottom-nav-guard`

移动端 dashboard 的视觉护栏只处理已有界面的可用性风险：900px 以下让固定底部导航使用 `env(safe-area-inset-bottom)` 定位，主容器为固定导航预留底部空间，核心锚点为底部导航预留滚动留白，避免 375px 小屏跳到 `归航台` 时遮挡归航按钮或内容底部。同时主资源数字改为按星尘核心卡片容器缩放，倍率 chip 改为专用两列 grid、固定宽度和等宽数字，避免桌面窄列或小屏下按 viewport 放大后挤压核心操作，也避免数字变化时带动倍率读回横向跳动。

约束：不新增面板、路由、玩法、资源、存档字段、导入 / 重置逻辑、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-return-event-route-feedback`

`星尘归航` 完成事件复用现有 `归航航线` 读回：如果归航后已经存在航线状态，事件记录会短读当前航线段、`completed/3`、本段进度和下一步行动，例如 `稳航校准 2/3 · 本段进度：归航 3/6 · 额外共鸣 2/4 · 下一步：继续重建工坊并执行 3 次星尘归航，同时保留 2 点额外共鸣`。这只改变即时反馈和本次 session 事件流，不改变归航奖励、存档、余辉公式或航线门槛。

约束：不新增资源、节点、等级树、任务系统、面板、存档字段、指标字段、视觉资产或 telemetry。
...
### `DECISION:2026-05-09-ui-followup-state-shells`

UI dashboard 后续补齐仍只做展示层：本次 session 事件流只读当前操作反馈，不写入存档；保存管理菜单只提供导出只读文本和导入 / 重置的禁用说明；研究所和日志增加锁定说明卡，解释后续方向但不开放系统。

约束：不新增真实研究系统、持久日志、任务、科技树、存档字段、telemetry、导入逻辑、重置逻辑或新玩法。
...
- `CONTENT_REVIEW`: 复核 `归航台` 航线短读是否减少玩家点击归航前的来回扫视。
