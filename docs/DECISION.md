...
v0.7 `归航航线` 第一段已把满节点后的重复归航组织成 3 段长期读回。当前主要问题转向复核航线读回是否足够清楚，而不是继续扩大节点数量或做节点等级树。
...
- `CONTENT_REVIEW`: 复核 `归航航线` 是否在真实或手动 playtest 中减少重复归航后的迷失感。
- `PLAYABLE_CONTENT`: 若复核后仍缺目标感，只做航线文案或门槛微调，不新增系统。
