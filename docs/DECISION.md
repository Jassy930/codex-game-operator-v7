...
v0.7 `归航航线` 已把重复归航组织成 3 段长期读回，并补齐本段进度、`深空信标储备`、具名差距、行动、收益、预览、节奏、航线图、摘要、航标化阶段目标和归航完成事件反馈。当前主要问题是保持这些读回不变吵：事件流应优先说明本次归航造成的变化，完整下一步行动继续由 `共鸣矩阵` 承载。下一步仍不应扩大节点数量或做节点等级树。
