...
v0.6 `归航余辉` 已能把满节点后的额外共鸣转成新一轮开局星尘助推，并读回可立即重建的自动采集器数量。当前主要问题是为 20 小时后的重复归航定义 v0.7 长线目标，而不是继续扩大节点数量或做节点等级树。
...
- `PLAYABLE_CONTENT`: 用 TDD 实现 v0.7 `归航航线` 第一段读回，仍复用现有 `共鸣矩阵` 和阶段目标。
