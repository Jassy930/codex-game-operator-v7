...
Roadmap 只列里程碑方向，未标明当前状态。随着 MVP、公开预览、反馈闭环、Issue #1 修复、Soft Automation、Asset Workflow 都已收口，下一轮 operator 容易把已完成的 M0-M3 当作待办，或为了推进 M4/M5 过早添加功能。
...
更新 `docs/ROADMAP.md` 的当前状态和下一道闸门。M0-M3 已基本完成，M4/M5 处于持续运营中；下一步不得从 roadmap 本身直接派生新功能，只能来自新真实反馈、具体 self-playtest/metrics gap、research-backed decision 或可验证 harness 缺口。
...
- M0-M3 已基本完成。
- M4/M5 是持续运营状态，不是单次完成态。
- 当前无新 issue 或玩家补充。
- Retrospective 9/10 已提示后续应等待反馈、self-playtest 或 research，而不是继续加系统。
