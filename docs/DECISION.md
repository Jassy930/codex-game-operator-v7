...
v0.7 `归航航线` 已把重复归航组织成 3 段长期读回，并补齐本段进度、`深空信标储备`、具名差距、行动、收益、预览、节奏、航线图、摘要、航标化阶段目标和归航完成事件反馈。当前主要问题转向 dashboard / 移动端可读性复核；下一步仍不应扩大节点数量或做节点等级树。

## Active Decisions

### `DECISION:2026-05-09-mobile-bottom-nav-guard`

移动端 dashboard 的视觉护栏只处理已有界面的可用性风险：900px 以下让固定底部导航使用 `env(safe-area-inset-bottom)` 定位，主容器为固定导航预留底部空间，核心锚点为底部导航预留滚动留白，避免 375px 小屏跳到 `归航台` 时遮挡归航按钮或内容底部。同时主资源数字改为按星尘核心卡片容器缩放，避免桌面窄列或小屏下按 viewport 放大后挤压核心操作。
