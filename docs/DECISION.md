...
v0.7 `归航航线` 已把重复归航组织成 3 段长期读回，并补齐本段进度、贯通储备、具名差距、行动、收益、预览、节奏、航线图、摘要和航标化阶段目标。当前主要问题转向可读性复核；下一步仍不应扩大节点数量或做节点等级树。

## Active Decisions

### `DECISION:2026-05-09-v07-route-stage-goal`

v0.7 `归航航线` 继续保持纯派生读回。本轮只新增阶段目标 `stageGoal`：未贯通时显示当前段、完成段数和进入下一段的缺口；三段贯通后显示长期储备数量。完整行动、收益、节奏、航线图和预览仍留在现有 `共鸣矩阵` 详情中。

约束：不修改归航奖励、余辉公式或 50 星尘上限；不新增资源、节点、等级树、任务系统、面板、存档字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-route-completion-reserve`

v0.7 `归航航线` 继续保持纯派生读回。本轮只调整三段贯通后的完成态：`routeProgress`、`routeSummary` 和 `currentPayoff` 会显示当前累计归航次数和额外共鸣储备，例如 `长期储备：6 次归航 / 5 点额外共鸣`。阶段目标继续保持短提示，避免主目标行膨胀。

约束：不修改归航奖励、余辉公式或 50 星尘上限；不新增资源、节点、等级树、任务系统、面板、存档字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-route-progress-meter`

v0.7 `归航航线` 继续保持纯派生读回。本轮只增加 `routeProgress`：未贯通时显示归航次数和额外共鸣的当前值 / 目标值，例如 `归航 3/6 · 额外共鸣 2/4`；三段贯通后显示 `航线 3/3 已贯通`。现有 `共鸣矩阵` 详情显示这行进度，阶段目标继续使用短行动提示。

约束：不修改归航奖励、余辉公式或 50 星尘上限；不新增资源、节点、等级树、任务系统、面板、存档字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-route-named-gap`

v0.7 `归航航线` 继续保持纯派生读回。本轮只把 `progressSummary` 和 `cadenceForecast` 里的抽象“下一段”替换为具名目标：第一段指向 `稳航校准`，第二段指向 `深空归航`。`routeSummary`、现有 `共鸣矩阵` 详情和阶段目标继续复用同一份派生读回。

约束：不修改归航奖励、余辉公式或 50 星尘上限；不新增资源、节点、等级树、任务系统、面板、存档字段、视觉资产或 telemetry。

### `DECISION:2026-05-09-v07-route-quantified-payoff`

v0.7 `归航航线` 继续保持纯派生读回。本轮只把现有摘要和当前收益里的抽象价值短语换成量化收益：复用 `归航余辉` 的起步星尘和自动采集器成本换算，在未贯通航线时读回“下轮起步可重建 N 台自动采集器”。完整门槛、行动提示、航线图、节奏预判和下一段预览继续保留在同一块内。
...
- `CONTENT_REVIEW`: 复核带本段进度、量化收益和贯通态储备数量的 `归航航线` 摘要是否在真实或手动 playtest 中减少重复归航后的迷失感。
