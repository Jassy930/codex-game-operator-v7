...
v0.7 `归航航线` 已把满节点后的重复归航组织成 3 段长期读回，并补齐距下一段还差多少归航次数和额外共鸣。当前主要问题转向复核这个动态读回是否足够清楚，而不是继续扩大节点数量或做节点等级树。
...
- `CONTENT_REVIEW`: 复核 `归航航线` 动态差距读回是否在真实或手动 playtest 中减少重复归航后的迷失感。
