...
Issue #2 目标提示切片已经发布并回复。发布后 self-playtest 显示首屏文案仍在预算内，但发现极小离线收益会被格式化为“离线获得 0 星尘”，这会削弱回访反馈质量。
...
Issue #2 目标提示切片发布后，首屏文案和布局仍在预算内。本轮不新增玩法，也不进入 `SIMPLIFY`。修复一个 self-playtest 发现的回访显示 bug：低于 `0.1` 的离线收益不展示提示，也不记录为 `offline_reward_claimed`，避免出现“离线获得 0 星尘”。
...
Self-playtest after Issue #2 release.
...
- Issue #2 release follow-up: `d2d2f94`
