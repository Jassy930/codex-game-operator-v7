...
首次回访后，玩家看到离线收益并连续购买可负担升级后，离线收益提示仍可见，但当前星尘可能已经低于下一次升级成本。当前最大风险不是玩法停滞，而是 `星尘引擎室` 的回访目标继续要求“花掉离线星尘”，指向玩家刚完成且暂时不能继续执行的动作。
...
Decision Anchor: `DECISION:2026-05-07-offline-return-after-spend-goal`

当离线收益实际可见且阶段为 `星尘引擎室` 时，继续按当前是否可负担升级区分目标：如果还能购买自动采集器或调校工具，提示“花掉离线星尘”；如果已经低于下一次升级成本，提示“离线收益已投入工坊，继续攒下一次升级”。本轮不新增第三/第四种升级、不新增第二资源、不新增面板、不修改存档或指标字段。
