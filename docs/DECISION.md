...
运行态文档已经开始影响迭代效率。`DECISION.md`、`SELF_PLAYTEST.md`、`RETROSPECTIVE.md` 和 `GOVERNOR_STATE.md` 承载了过多历史细节，容易让 operator 把旧阶段当成当前目标。本轮将文档治理统一成机制：先归档原文，再把运行态文档压缩到当前状态和高信号摘要。
...
- `CONTENT_REVIEW`: 复核归航后第 2 轮体验、可用共鸣消费目标和重复归航节奏。
- `PLAYABLE_CONTENT`: 如果 review 发现明确 gap，再做归航后读回或节点消费体验整理。
