...
首次回访后，玩家已经看到离线收益并且有足够星尘继续购买升级，但 `星尘引擎室` 的阶段目标仍停留在“离开一会儿再回来”。当前最大风险不是玩法停滞，而是回访首屏没有把离线收益自然接回“花掉收益、继续扩建或调校”的核心循环。
...
Decision Anchor: `DECISION:2026-05-07-offline-return-spend-goal`

只在离线收益实际可见且阶段为 `星尘引擎室` 时，把同一阶段目标行改为“回访目标：花掉离线星尘，继续扩建或调校引擎室”。本轮不新增第三/第四种升级、不新增第二资源、不新增面板、不修改存档或指标字段。
