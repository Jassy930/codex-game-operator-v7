...
- M4 持续运营：进行中，v0.3 / 共鸣系统第一版已实现；首次共鸣可领取、已领取状态、选择前、选择后、节点启动后、回访线圈生效后、调校刻印调校反馈、稳定回路自动采集器反馈、首个共鸣后 8 小时时间窗复核和 `回访计划读回` 已收敛到下一升级与大致等待时间，前 60 秒作为回归护栏保留。本地指标快照入口已补齐，并能读回活跃 session 当前持续时长，方便后续读取共鸣触达证据。当前暂不扩展更多共鸣系统。
...
- research-backed 决策，且通过 `docs/COMPLEXITY_BUDGET.md` 和 `docs/DECISION.md`；`回访计划读回` 第一版已落地，后续不能把它扩成新面板或新系统。
...
允许的下一步仍必须是小切片：单个节点数值复核、local-only metrics snapshot readback、self-playtest 复核、恢复远端反馈读取或等待真实反馈。当前 metrics snapshot readback 已能返回 `activeSessionDurationMs`；后续不要继续扩 metrics 字段，除非有新的可解释运营缺口。禁止把 v0.3 直接扩成 prestige、任务系统、多生产线、更多节点、第二个共鸣里程碑或多个新面板。
...
最近阶段切换：用户明确要求更丰富玩法后，v0.3 已实现为 `共鸣系统` 第一版；随后 self-playtest 已收敛首个共鸣可领取、首个共鸣已领取后的进度状态、首次节点选择前、选择后、首个已选节点后的目标文案、`回访线圈` 在可见离线收益状态下的节点价值归因、`调校刻印` 的调校购买反馈，以及 `稳定回路` 的自动采集器购买反馈。首个共鸣后 8 小时主动模拟仍有升级推进，但没有新的可复现扩展依据。`回访计划读回` 已复用现有阶段目标解释“已选节点在帮你等什么、下一步大约等多久”；本地指标快照读回入口已补齐，并可读回活跃 session 当前持续时长。后续可优先读取 `window.stardustWorkshopMetricsSnapshot()` 证据，但仍不能直接进入 prestige、任务系统、多生产线、更多节点或多个新面板。
