...
- M4 持续运营：进行中，当前阶段焦点切换为 3-5 分钟参与度；前 60 秒作为回归护栏保留。
- M5 可持续演化：进行中，harness 已通过 meta-governance 增加反馈闭环检查和 Asset Workflow。

## 下一道闸门

当前进入 3-5 分钟阶段，但不要因为阶段切换就直接新增第三种升级、第二资源、图片资产或新面板。下一步只能来自：

- 新 issue 或 Issue #1 的玩家补充。
- 新 self-playtest gap，且能追溯到 3-5 分钟目标清晰度、成长反馈、回访体验或前 60 秒回归风险。
...
- 第 1 分钟后玩家是否仍知道下一个目标。
- 第 3-5 分钟内是否持续出现可理解的升级或回访目标。
- 调校工具是否让“数量 vs 效率”的选择更清晰，而不是让下一台自动采集器显得遥远。
- 离线收益和本地 session history 是否足以支持回访判断。

允许的下一步仍必须是小切片：目标提示、节奏解释、现有升级的轻量反馈、metrics readback 或 no-change 决策。禁止把阶段切换直接解释为第二资源、prestige、任务系统或新面板授权。
...
- 如果扩大时间窗，下一轮检查必须写清新的目标范围，例如 `5-15 分钟`、`15-60 分钟` 或 `首次回访`。
- 如果定义内容弧线，先写文档，不直接实现系统。
- 如果等待真实反馈，不继续产出重复 no-change 文档；转向反馈获取、metrics readback 或暂停新增玩法。

当前 3-5 分钟阶段最多允许两轮无改动复核。若仍无具体 gap，下一步应定义 `docs/CONTENT_ARC.md`，覆盖 0-60 分钟和首次回访目标。
