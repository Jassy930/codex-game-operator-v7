...
当前最大体验问题是 v0.5 `星尘归航` 已经接上 20 小时主循环，但移动端动作按钮在纵向堆叠时仍继承桌面 flex basis，首屏操作区显得过重，会削弱早期操作节奏。

## Evidence

- 2026-05-08 visual polish 复核：`.action-row` 在 560px 以下会纵向排列，但 `.primary-action` 和 `.upgrade-action` 仍继承桌面 `flex: 1 1 180px` 的基准，导致移动端按钮高度不够紧凑。
- 当前已有星尘、自动采集器和调校工具插图；问题不需要新增图片素材，只需要修正移动端按钮布局和禁用态层级。
- Decision: 本轮记录 `DECISION:2026-05-08-mobile-button-visual-polish`。在 560px 以下让主操作和升级按钮使用 `width: 100%`、`min-height: 48px`、`padding: 12px`、`flex-basis: auto`，升级按钮左对齐；禁用升级按钮中的插图轻度降饱和和透明。
- 约束：不新增玩法、数值、资源、按钮、面板、存档字段、指标字段、图片素材或 telemetry；不改变 v0.5 归航逻辑。
...
- 更新 `governor-check` 机器校验，要求 v0.5 预算存在、归航 loop 明确受限、奖励资源保持为 `共鸣`、存档版本上限为 3，并显式禁止第三普通资源。
- 将 `星图巡航` 降级为后续辅助计划，v0.5 主线改为 `星尘归航`。
- 第一版仍禁止第三普通资源、任务系统、复杂地图、多生产线、多个新面板、第三共鸣门槛、新共鸣节点、节点等级树和外部 telemetry。
