...
目标：清理移动端首屏动作按钮的视觉节奏，避免按钮纵向堆叠时异常拉高。
Appetite：1 个视觉 polish 切片。
包括：移动端按钮 CSS 覆盖、禁用态插图层级、CSS 回归测试、视觉 polish 计划和发布记录。
不包括：不新增玩法、数值、资源、按钮、面板、存档字段、指标字段、图片素材或外部 telemetry。
完成定义：CSS 回归测试、完整测试、构建、governor check 和 whitespace 检查通过；记录浏览器验证缺口并把下一轮候选切回可玩内容。

## Expected Content Advance

移动端按钮视觉打磨：窄屏纵向排列时主操作和升级按钮使用稳定宽度、紧凑高度和 `flex-basis: auto`，禁用升级按钮的插图状态更清楚。

## Evidence Source

现有视觉资产已接入后，移动端首屏动作区在窄屏纵向堆叠时仍继承桌面按钮的 flex basis，导致按钮高度和操作节奏偏重。该问题属于当前游戏图像表现整理优化，不需要新增素材或玩法系统。

## Required Artifact

更新 `src/styles.css`、`src/ops-scripts.test.ts`、`docs/DECISION.md`、`docs/RELEASE_LOG.md` 和移动端按钮视觉计划文档。
...
v0.5 归航第一版已经把 20 小时主循环接上，当前移动端视觉阻尼会影响首屏操作密度。先用轻量 CSS 收口，不改变内容系统，避免视觉问题干扰下一轮可玩内容复核。

## Allowed Actions

- 修改移动端按钮 CSS，使纵向堆叠时不继承桌面 flex basis。
- 为禁用升级按钮插图增加轻量状态层级。
- 用 CSS 文本回归测试锁定移动端按钮覆盖。
- 更新 visual polish 决策、release 记录和 governor state。
- 运行本地验证，至少包括聚焦测试、完整测试、构建、`./ops/governor-check.sh` 和 `git diff --check`。

## Forbidden Actions

- 本轮不实现新的游戏玩法、数值、按钮、面板、存档字段或指标字段。
- 不新增图片素材，不接入 imagegen，不改变现有资产路径。
...
- 移动端按钮 CSS 回归测试覆盖 `flex-basis: auto`。
- 完整测试、构建、governor check 和 whitespace 检查通过。
- 文档和工作区状态已收口。

## Next Candidate Mode / Track

下一轮优先回到 `OPERATE / PLAYABLE_CONTENT` 或 `SELF_PLAYTEST / CONTENT_REVIEW`：复核归航后的第 2 轮体验、可用共鸣消费目标和重复归航节奏。不要继续停留在视觉 polish，除非本地截图或玩家反馈明确指出视觉回退。

## Drift Status

未发现复杂度漂移：本轮只改现有按钮 CSS 和测试，不新增玩法、资源、按钮、面板、存档字段、指标字段或素材。

## Last Updated

2026-05-08: 移动端按钮视觉 polish 完成；`src/styles.css` 在 560px 以下覆盖主操作和升级按钮的宽度、最小高度、padding 与 `flex-basis: auto`，并让禁用升级按钮插图轻度降饱和。验证通过：`bun test src/ops-scripts.test.ts` 12 pass，`bun test` 98 pass，`bun run build` 成功，`./ops/governor-check.sh` 退出 0，`git diff --check` 退出 0。验证缺口：浏览器插件连接超时，当前沙箱对本地 loopback 连接返回 `Operation not permitted`，未能完成截图读回。下一轮候选回到 `PLAYABLE_CONTENT` 或 `CONTENT_REVIEW`，复核归航后的第 2 轮体验。
