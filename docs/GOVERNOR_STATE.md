...
2026-05-08: v0.4 双节点组合读回切片已由 commit `47c5fab` 本地提交。新增测试先按预期失败，随后本地验证通过：`bun test src/App.test.tsx -t "names the active dual-node cruise combination"` 1 pass，回归测试 `bun test src/App.test.tsx -t "dual-node|choice cap"` 3 pass，`bun test src/App.test.tsx` 33 pass，完整 `bun test` 83 pass，`bun run test` 83 pass，`bun run build` 成功，`./ops/governor-check.sh` 退出 0，`git diff --check` 退出 0。远端 issue 刷新缺口：`gh issue list --repo Jassy930/codex-game-operator-v7 --state open --limit 20 --json ...` 无法连接 `api.github.com`。
