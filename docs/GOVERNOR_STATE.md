...
2026-05-09: 完成 UI dashboard 移动端视觉护栏。900px 以下 `.app-shell` 改为安全区感知底部留白，`.bottom-nav` 使用 `env(safe-area-inset-bottom)` 定位，`引擎室` / `共鸣室` / `归航台` 锚点补底部滚动留白；同时主资源数字改为按星尘核心卡片容器缩放，避免窄列和小屏下挤压核心操作。不改玩法、存档、归航、共鸣或导入 / 重置语义。验证通过 `bun test`、`bun run test`、`bun run build`、`./ops/governor-check.sh` 和 `git diff --check`。Browser 没有可用 pane，且本轮 shell 无法连接已监听的本地 Vite 端口，真实截图复核仍是验证缺口。
