...
Issue #2 最小切片发布后，复核首屏文案预算、窄屏布局和回访提示质量，确认目标提示改动没有引入新的 UI 噪音或显示异常。

## Allowed Actions

- 复核当前首屏文案预算。
- 复核 `.action-row` 和窄屏布局约束。
- 修正 self-playtest 发现的低风险显示 bug。
- 更新 `docs/SELF_PLAYTEST.md`、`docs/DECISION.md` 和 release 记录。
- 运行测试、构建、治理检查和低成本验证。
...
- 不基于同一轮 self-playtest 继续扩张玩法。
- 不重复回复 Issue #1 或 Issue #2，除非玩家提供新的实质信息。

## Exit Criteria

- 首屏文案仍低于 `docs/COMPLEXITY_BUDGET.md` 的 300 中文字符预算。
- 窄屏动作区仍有 CSS 换行/纵向排列保护。
- 不再显示会被四舍五入成 0 的离线收益提示。
- 测试、构建、governor check 和 diff check 通过。
- `./ops/governor-check.sh` 通过。
- 周期结束后工作区状态已记录。

## Drift Status

未发现玩法漂移。本轮只做 post-release self-playtest 和一个显示阈值修复，不新增系统、不改变经济、不扩张 UI。
