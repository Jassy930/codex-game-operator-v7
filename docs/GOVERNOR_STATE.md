...
v0.3 共鸣系统第一版已经发布，首次节点选择前后的约束和状态文案已收敛。本轮继续按 roadmap 闸门进入 SELF_PLAYTEST，复核玩家达到首个共鸣门槛、尚未领取共鸣时，主目标是否会把玩家指向当前可执行动作。

## Allowed Actions

- 只读模拟前 60 秒、3-15 分钟、15-60 分钟和首个共鸣门槛状态。
- 若发现具体 gap，仅实施一个可测试的小切片，优先改善共鸣门槛达成后的目标提示或本地指标可读性。
...
- 前 60 秒仍不显示 `共鸣矩阵`。
- 共鸣门槛、可领取共鸣状态、首次共鸣后的节点选择状态和选择前约束已复核。
