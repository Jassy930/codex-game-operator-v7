...
v0.3 共鸣系统第一版已经发布，上一轮已收敛首次节点选择后的已选/未选状态。本轮继续按 roadmap 闸门进入 SELF_PLAYTEST，复核玩家在拥有 1 点共鸣、尚未选择节点时，是否能理解这是 v0.3 的一次性三选一永久选择。

## Allowed Actions

- 只读模拟前 60 秒、3-15 分钟、首次共鸣前后状态。
- 若发现具体 gap，仅实施一个可测试的小切片，优先改善共鸣选择前说明或本地指标可读性。
