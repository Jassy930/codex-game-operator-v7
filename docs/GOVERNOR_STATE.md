...
目标：复核 UI dashboard 视觉稳定性，确保星尘核心读数、倍率 chip 和 375px 小屏底部导航不会遮挡或带动核心操作读回。
Appetite：1 个 CSS 护栏小切片。
包括：移动端底部导航安全区、底部留白、锚点滚动可见性、星尘核心数字 / 倍率稳定性护栏、窄测试和运行态文档同步。
不包括：新增玩法、资源、节点、任务、面板、存档字段、telemetry、导入 / 重置逻辑或新视觉资产。
完成定义：移动端底部导航使用安全区感知定位，主容器为固定导航预留足够底部空间，锚点跳转到 `归航台` 时不会被底部导航压住；星尘核心数字按卡片缩放，倍率 chip 有稳定锚点和宽度。
...
更新 `src/App.tsx`、`src/styles.css`、`src/ops-scripts.test.ts`、`docs/SELF_PLAYTEST.md`、`docs/DECISION.md`、`docs/RELEASE_LOG.md` 和本文件。
