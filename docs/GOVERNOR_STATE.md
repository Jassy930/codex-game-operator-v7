...
RESEARCH 已产出 `DECISION:2026-05-07-research-soft-automation` 和设计计划。本轮进入 SELF_PLAYTEST 的实施切片，只实现一个低复杂度第二升级类型 `调校工具`，并复核它是否仍在复杂度预算内。

## Allowed Actions

- 实现 `调校工具` 升级：使用星尘提升自动采集器效率。
- 保持单一资源、现有主屏和同一反馈渠道。
- 添加 TDD 覆盖游戏逻辑、旧存档兼容和 UI 文案。
- 更新 `docs/DECISION.md`、`docs/SELF_PLAYTEST.md`、`docs/RELEASE_LOG.md`。
...
- `调校工具` 有游戏逻辑、存档兼容和 UI 测试。
- `bun test`、`bun run test`、`bun run build`、`./ops/governor-check.sh` 通过。
- 复杂度预算仍满足：primary resource 1，upgrade types 不超过 3，visible panels 不增加。
- 周期结束后记录工作区状态。

## Drift Status

未发现玩法漂移。本轮只增加一个 research-backed upgrade type，不新增资源、面板、反馈渠道或上传路径。
