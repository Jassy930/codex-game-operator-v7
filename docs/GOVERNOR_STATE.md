...
Issue #1/#2 没有新玩家补充，不能重复回复或继续把同一条反馈当成新任务。v0.2 已完成工坊阶段、最近升级进度、阶段完成反馈和延后解锁预告；本轮切到 SELF_PLAYTEST，复核 15-60 分钟 / 首次回访目标。具体 gap：达到 `星尘引擎室` 后，下一步文案仍像 operator 备注，而不是玩家可执行的长期目标。

## Allowed Actions

- 使用现有 `getWorkshopStage(autoCollectors, autoCollectorEfficiencyLevel)` 输出引擎室后的长期目标。
- 只调整 `星尘引擎室` 的 `nextRequirement` 文案，不新增 UI 区块。
- 保持自动采集器、调校工具、阶段行、进度条、存档和指标结构不变。
- 更新 `docs/DECISION.md`、`docs/SELF_PLAYTEST.md`、`docs/CONTENT_ARC.md`、`docs/RELEASE_LOG.md` 和 `docs/ROADMAP.md`。
...
- 不新增第三或第四种升级类型；本轮只做引擎室后的回访目标文案。
- 不修改 Issue #1/#2 回复，除非玩家在 issue 中提供新实质信息。
- 不放宽 issue routing、response budget、review protocol、测试或部署要求。

## Exit Criteria

- `docs/DECISION.md` 说明本轮使用 v0.2 的 delayed unlock copy / first-return 预算且不破坏 v0.1 回归护栏。
- `星尘引擎室` 后续目标改为玩家可理解的首次回访目标，不出现 operator 观察备注。
- 购买反馈、阶段行、进度条和升级按钮行为保持不变。
- `docs/SELF_PLAYTEST.md` 记录 15-60 分钟 / 首次回访目标检查。
- `bun test`、`bun run test`、`bun run build`、governor check 和 diff check 通过。
- 周期结束后工作区状态已记录。

## Drift Status

未发现玩法漂移。本轮只改现有阶段文案，不新增第二资源、prestige、任务系统、新面板、外部 analytics、存档字段、指标字段或 issue 回复。
